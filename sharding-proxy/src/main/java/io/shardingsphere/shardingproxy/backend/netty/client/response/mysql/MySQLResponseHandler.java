...
public final class MySQLResponseHandler extends ResponseHandler {
    ...
    public MySQLResponseHandler(final String dataSourceName, final String logicSchemaName) {
        dataSourceParameter = GLOBAL_REGISTRY.getLogicSchema(logicSchemaName).getDataSources().get(dataSourceName);
        dataSourceMetaData = GLOBAL_REGISTRY.getLogicSchema(logicSchemaName).getMetaData().getDataSource().getActualDataSourceMetaData(dataSourceName);
    ...
    private void setResponse(final ChannelHandlerContext context, final MySQLQueryResult queryResult) {
        final QueryResultCollector queryResultCollector = ChannelRegistry.BACKEND_CHANNEL_QUERY_RESULT_COLLECTOR.get(ChannelUtils.getLongTextId(context.channel()));
    ...
    private void triggerFinish(final ChannelHandlerContext context) {
        ...
        if (queryResultCollector.getSqlStatement().getType() == SQLType.DQL) {
            ShardingProxy.getInstance().getUserGroup().execute(new QueryResultExecutor(queryResultCollector));
        } else {
    ...
    private void release(final Channel backendChannel) {
        backendChannelResultMap.remove(ChannelUtils.getLongTextId(backendChannel));
        ChannelPoolMap channelPoolMap = BackendNettyClientManager.getInstance().getBackendNettyClient(logicSchemaName).getEventLoopChannelPoolMap().get(backendChannel.eventLoop());
